package w.expenses8.web.servlet;
...
public class FileServlet extends HttpServlet {
   ...
   private final Path baseRepository;

   private final String realPath;

   public FileServlet(String root) {
	   this.baseRepository = Paths.get(root).normalize().toAbsolutePath();
      if (!Files.exists(this.baseRepository)) {
         log.warn("{} does not exist!", this.baseRepository );
      } else if (!Files.isReadable(this.baseRepository)){
         log.warn("{} is not readable!", this.baseRepository);
      } else {
    	  log.info("Reading files from {}", this.baseRepository);
      }
      
      this.realPath = this.baseRepository.toString();
   }
   
   public Path getFilePath(String filename) {
      Path file = baseRepository.resolve(filename).normalize().toAbsolutePath();
      if (file.toString().startsWith(realPath)) {
         log.debug("Retrieving file {}",file);
         return file;
      } else {
         log.warn("File {} is not in {}",file, realPath);
         return null;
      }
   }
   ...
   private void processRequest(HttpServletRequest request, HttpServletResponse response, boolean content) throws IOException {
      ...
      String filename = URLDecoder.decode(requestedFile.substring(1) /* remove leading slash */, "UTF-8");
      Path filepath = getFilePath(filename);
      if (filepath == null) {
         log.warn("File [{}] is not accessible", filename);
